...
def get_urls():
    ...
    for item in classes:
        ...
        or ("CEE" in item) or ("ADF" in item) or ("BME" in item)
        or ("CHURMIN" in item) or ("RON" in item) or ("XTIAN" in item)):
            del(item)
            continue
        s = list(item)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        electives = get_urls()
        # electives = ["https://streamer.oit.duke.edu/curriculum/courses/subject/MENG%20-%20Master%20of%20Engineering?access_token=94fbc0640ed3e6e2e70f942dd4867a33"]
        for url in electives:
            ...
            for dict in mylist:
                if "L9" in dict['catalog_nbr']:
                    ...
                    # logger.error("Listing is:"+listing)
                    # logger.error("Title is:"+title)
                    # logger.error("Term is:"+term)
