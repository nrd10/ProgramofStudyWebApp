...
This function also handles possible errors returned by the Duke API such as if 1) the API returns NULL if there are no classes for a specific subject or 2) if the API returns a 404 status code. In these scenarios, if we attempted to import courses without any data, the Django application will throw an error. This Course Import functionality will catch these error cases and skip over the subjects that have no data to import. The functionality will then proceed to continue to attempt to import courses of different subjects. 
...
when our `posweb-init` Docker container is spun up:
...
to be ran in the background. More information on Celery is located in [Asynchronous and Background Tasks](#asynchronous-and-background-tasks).
...
in the background. More information on Celery is located in [Asynchronous and Background Tasks](#asynchronous-and-background-tasks).
